...
> **Pro tip:** Pro and Team users can skip Ollama entirely — the plugin auto-routes summarization to the hosted cloud LLM (xAI Grok 4.3) once `~/.ashlr/pro-token` is present. See `/ashlr-upgrade`. Ollama remains the recommended path for free tier and offline use.
