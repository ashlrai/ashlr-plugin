...
The session log is the source of truth for `/ashlr-usage`, `/ashlr-dashboard`, and `/ashlr-coach`. It is never read by the MCP servers during a session — only written.
