...
Practical effect: small configs, short utility modules, and stub files are always returned in full. The large-read savings apply to the ≥2 KB slice only. See [savings math](/docs/concepts/savings-math) for the honest breakdown.
...
The cross-repo, cross-language headline (TypeScript + Python + Rust) is **−57%**. Large-file reads remain the highest-savings case, while small files are returned in full. See [savings math](/docs/concepts/savings-math) for methodology.
