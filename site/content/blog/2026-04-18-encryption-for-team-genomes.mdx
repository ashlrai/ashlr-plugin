...
date: "2026-04-18"
author: "Mason Wyatt"
description: "v1.8.0 shipped client-side AES-256-GCM encryption for shared team genomes. This post walks through the threat model, how keys are managed, and how encryption pairs with the CRDT sync layer."
...
v1.8.0 shipped client-side AES-256-GCM encryption for the genome store. This post explains the threat model we designed against, how keys work in practice, and how the encryption layer interacts with the CRDT sync that keeps the genome consistent across team members.
...
## Integration with CRDT sync

The team genome uses a CRDT (Conflict-free Replicated Data Type) to merge concurrent edits from multiple team members. Each section has a vector clock, and the server applies a last-write-wins rule at the section level when clocks are incomparable.
...
The CRDT merge handles this correctly because it operates on the opaque `{ nonce, ciphertext }` envelope, not on the plaintext content. The last writer wins based on the vector clock comparison, and the losing update is discarded. On the client side, after the merge, the winning envelope is decrypted and rendered.

One subtle case: if member A and member B make semantically identical edits (type the same content) but are at different vector clock positions, the merge picks one envelope arbitrarily (the higher clock wins). After decryption both produce the same plaintext, so the user sees no conflict. This is harmless but slightly wasteful — a future optimization could detect identical-plaintext conflicts at merge time and skip the loser earlier.
