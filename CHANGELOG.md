...
- **All six remaining filesystem-touching MCP tools now refuse paths outside `process.cwd()`.** Previously a caller could pass `cwd: "/etc"`, `path: "/etc/passwd"`, or `path: "~/.ssh/id_rsa"` and the tool would dutifully walk the filesystem, read the file, or (for the edit tools) overwrite it. The attack surface was identical to the v1.11.1 `ashlr__ls` gap — same input shape, same trust boundary, same disclosure or write-primitive outcome. The fix routes all seven FS tools (`ashlr__ls` + `ashlr__glob`, `ashlr__tree`, `ashlr__grep`, `ashlr__read`, `ashlr__edit`, `ashlr__multi_edit`) through a new `clampToCwd()` helper at `servers/_cwd-clamp.ts` that resolves symlinks via `realpathSync` (so `/var/folders/…` and `/private/var/folders/…` are treated as the same directory on macOS) and returns a refusal message matching the ls-server convention. `ashlr__grep`'s `findParentGenome` walk-up for inherited genomes is preserved — only the ripgrep spawn target is clamped, so legitimate monorepo-parent genome inheritance still works. `ashlr__multi_edit` clamps *before* reading any file so a bad path in a batch can't partially apply edits.
